...
func (c *calling) run() (err error) {
	...
	if !c.quiet {
		...
		if err != nil {
			err = fmt.Errorf("system call failed (command-line: %q): %w", c.Args, err)
		}
	}
	return
}
...
func (c *calling) runNow() error {
	if c.Cmd == nil {
		return errors.New("has WithCommand() not called yet")
	}
	...
	if c.err = c.Cmd.Wait(); c.err != nil {
		...
		if ok {
			...
			c.err = fmt.Errorf("%q failed with stderr:\n%v\n  %w", c.Path, c.slurp.String(), c.err)
			return c.err
		}
